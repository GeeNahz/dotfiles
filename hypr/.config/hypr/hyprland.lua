...
hl.on("hyprland.start", function()
  ...
  hl.exec_cmd(
    [[sh -c 'sleep 1 && hyprctl keyword cursor:no_hardware_cursors 0 && sleep 0.5 && hyprctl keyword cursor:no_hardware_cursors 2']])
end)
...
hl.bind(mainMod .. " + ALT + S",
  hl.dsp.exec_cmd(
    [[sh -c 'mkdir -p ~/Pictures/Screenshots && FILE=~/Pictures/Screenshots/$(date +%Y%m%d_%H%M%S).png && grim "$FILE" && wl-copy < "$FILE"']]))
...
hl.bind(mainMod .. " + ALT + CTRL + S",
  hl.dsp.exec_cmd(
    [[sh -c 'mkdir -p ~/Pictures/Screenshots && grim -g "$(slurp)" ~/Pictures/Screenshots/$(date +%Y%m%d_%H%M%S).png']]))
